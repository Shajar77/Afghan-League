...
export function RegisterPage() {
  ...
  // Private Access Guard Check (Option B)
      ...
      if (currentStep < 5) {
        ...
        try {
          ...
          const formattedProfileLink = rawProfileLink
            ? (/^https?:\/\//i.test(rawProfileLink) ? rawProfileLink : `https://${rawProfileLink}`)
            : ''
          ...
          const regPayload = {
            ...
            // For spin bowlers: spinType ("Off Spinner") is the real bowling_type value.
            // For fast bowlers / all-rounders: use bowlingSubtype or bowlerType.
            bowling_type: formData.spinType || formData.bowlingSubtype || formData.bowlerType || 'None',
            // For spin bowlers: "Spin Bowler - Off Spinner", for others: "Fast Bowler"
            bowler_category: formData.bowlerType && formData.spinType
              ? `${formData.bowlerType} - ${formData.spinType}`
              : formData.bowlerType || 'None',
            // Specific spin variety — stored separately so admin panel can display it
  ...
  if (!isAuthorized) {
    return (
      <div className="register-page" style={{ paddingTop: '8rem', paddingBottom: '8rem', minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          maxWidth: '640px',
          width: '100%',
          margin: '0 auto',
          ...
          <p style={{
            ...
            Official player registration for the Afghanistan Premier League T20 (APL T20) is strictly private and by invitation only. If you are an official player or registered agent, please use the private invitation link provided directly to you.
          </p>
